...
export default function AdminSettingsPage() {
  ...
  const handleSyncKnowledge = async () => {
    ...
    try {
      ...
      toast.success('Supervity Brain Updated', { description: 'Corporate policies have been synced and cached.' })
      setKnowledgeText('')
    } catch (err) {
      toast.error('Sync Failed', { description: 'Could not connect to Supervity Ingestion Ops.' })
    } finally {
      setIsSyncing(false)
    }
  }

  if (sessionStatus === 'loading') return <div className='flex h-full items-center justify-center'><Icons.loader className='h-8 w-8 animate-spin' /></div>
    ...
    <div className='space-y-6'>
      ...
      <Card className='relative overflow-hidden'>
        ...
        <CardHeader className='relative z-10'>
          ...
          <CardDescription>Paste raw corporate documents to train the Aegis-Nexus brain (No-Script Mode).</CardDescription>
        </CardHeader>
        <CardContent className='relative z-10 space-y-4'>
          <textarea
            ...
            className="w-full min-h-[250px] p-4 rounded-xl border border-border bg-white text-sm focus:ring-2 focus:ring-brand-cornflower/50 outline-none"
          />
